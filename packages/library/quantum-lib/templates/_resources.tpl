{{- define "quantum-lib.resources.defaultCpuAllocationRatio" }}
{{-   `10` }}
{{- end }}

{{- define "quantum-lib.resources.cpuAllocationRatio" }}
{{-   include "quantum-lib.loadQuantumConfig" . }}
{{-   $quantumConfig := index . 1 "quantumConfig" }}
{{-   if not $quantumConfig }}
{{-     include "quantum-lib.resources.defaultCpuAllocationRatio" . }}
{{-   else }}
{{-     dig "data" "cpu-allocation-ratio" (include "quantum-lib.resources.defaultCpuAllocationRatio" dict) $quantumConfig }}
{{-   end }}
{{- end }}

{{- define "quantum-lib.resources.toFloat" -}}
    {{- $value := . -}}
    {{- $unit := 1.0 -}}
    {{- if typeIs "string" . -}}
...
{{- /*
  ...
  {{ include "quantum-lib.resources.sanitize" .Values.resources }}.
  Example input:
  ==============
  limits:
    cpu: "1"
...
*/}}
{{- define "quantum-lib.resources.sanitize" }}
{{-   $cpuAllocationRatio := include "quantum-lib.resources.cpuAllocationRatio" . | float64 }}
{{-   $sanitizedMap := dict }}
{{-   $args := index . 0 }}
...
{{-     else }}
{{-       $vcpuRequestF64 := (include "quantum-lib.resources.toFloat" $v) | float64 }}
