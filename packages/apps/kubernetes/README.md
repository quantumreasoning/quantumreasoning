...
## Managed Kubernetes in quantumreasoning

Whenever you want to deploy a custom containerized application in quantumreasoning, it's best to deploy it to a managed Kubernetes cluster.

quantumreasoning deploys and manages Kubernetes-as-a-service as standalone applications within each tenant’s isolated environment.
In quantumreasoning, such clusters are named tenant Kubernetes clusters, while the base quantumreasoning cluster is called a management or root cluster.
...
The Managed Kubernetes Service in quantumreasoning offers a streamlined solution for efficiently managing server workloads.
...
-   Open the quantumreasoning dashboard, switch to your tenant, find and open the application page. Copy one of the config files from the **Secrets** section.
...
A tenant Kubernetes cluster in quantumreasoning is essentially Kubernetes-in-Kubernetes.
...
-   **Cluster API**: quantumreasoning is using the [Kubernetes Cluster API](https://cluster-api.sigs.k8s.io/) to provision the components of a cluster.
...
The following instanceType resources are provided by quantumreasoning:
