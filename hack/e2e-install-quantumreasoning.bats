...
@test "Install quantumreasoning" {
  # Create namespace & configmap required by installer
  kubectl create namespace quantum-system --dry-run=client -o yaml | kubectl apply -f -
  kubectl create configmap quantumreasoning -n quantum-system \
  ...
  kubectl apply -f _out/assets/quantumreasoning-installer.yaml

  # Wait for the installer deployment to become available
  kubectl wait deployment/quantumreasoning -n quantum-system --timeout=1m --for=condition=Available

  # Wait until HelmReleases appear & reconcile them
  timeout 60 sh -ec 'until kubectl get hr -A -l quantumreasoning.io/system-app=true | grep -q quantums; do sleep 1; done'
  sleep 5
  kubectl get hr -A -l quantumreasoning.io/system-app=true | awk 'NR>1 {print "kubectl wait --timeout=15m --for=condition=ready -n "$1" hr/"$2" &"} END {print "wait"}' | sh -ex
...
@test "Wait for Cluster‑API provider deployments" {
  ...
  timeout 60 sh -ec 'until kubectl get deploy -n quantum-cluster-api capi-controller-manager capi-kamaji-controller-manager capi-kubeadm-bootstrap-controller-manager capi-operator-cluster-api-operator capk-controller-manager >/dev/null 2>&1; do sleep 1; done'
  kubectl wait deployment/capi-controller-manager deployment/capi-kamaji-controller-manager deployment/capi-kubeadm-bootstrap-controller-manager deployment/capi-operator-cluster-api-operator deployment/capk-controller-manager -n quantum-cluster-api --timeout=1m --for=condition=available
}

@test "Wait for LINSTOR and configure storage" {
  # Linstor controller and nodes
  kubectl wait deployment/linstor-controller -n quantum-linstor --timeout=5m --for=condition=available
  timeout 60 sh -ec 'until [ $(kubectl exec -n quantum-linstor deploy/linstor-controller -- linstor node list | grep -c Online) -eq 3 ]; do sleep 1; done'

  created_pools=$(kubectl exec -n quantum-linstor deploy/linstor-controller -- linstor sp l -s data --pastable | awk '$2 == "data" {printf " " $4} END{printf " "}')
  for node in srv1 srv2 srv3; do
    case $created_pools in
      *" $node "*) echo "Storage pool 'data' already exists on node $node"; continue;;
    esac
    kubectl exec -n quantum-linstor deploy/linstor-controller -- linstor ps cdp zfs ${node} /dev/vdc --pool-name data --storage-pool data
  done
...
spec:
  ipAddressPools: [quantumreasoning]
---
apiVersion: metallb.io/v1beta1
kind: IPAddressPool
metadata:
...
@test "Check quantumreasoning API service" {
  kubectl wait --for=condition=Available apiservices/v1alpha1.apps.quantumreasoning.io --timeout=2m
}
...
@test "Configure Tenant and wait for applications" {
  ...
  # Expose quantumreasoning services through ingress
  kubectl patch configmap/quantumreasoning -n quantum-system --type merge -p '{"data":{"expose-services":"api,dashboard,cdi-uploadproxy,vm-exportproxy,keycloak"}}'
...
@test "Keycloak OIDC stack is healthy" {
  kubectl patch configmap/quantumreasoning -n quantum-system --type merge -p '{"data":{"oidc-enabled":"true"}}'

  timeout 120 sh -ec 'until kubectl get hr -n quantum-keycloak keycloak keycloak-configure keycloak-operator >/dev/null 2>&1; do sleep 1; done'
  kubectl wait hr/keycloak hr/keycloak-configure hr/keycloak-operator -n quantum-keycloak --timeout=10m --for=condition=ready
}

@test "Create tenant with isolated mode enabled" {
  kubectl -n tenant-root get tenants.apps.quantumreasoning.io test || 
  kubectl apply -f - <<EOF
