#!/usr/bin/env bats
# -----------------------------------------------------------------------------
# quantumreasoning end‑to‑end provisioning test (Bats)
# -----------------------------------------------------------------------------

@test "Required installer assets exist" {
  if [ ! -f _out/assets/quantumreasoning-installer.yaml ]; then
    echo "Missing: _out/assets/quantumreasoning-installer.yaml" >&2
    exit 1
  fi
...
@test "Prepare networking and masquerading" {
  ip link del quantum-br0 2>/dev/null || true
  ip link add quantum-br0 type bridge
  ip link set quantum-br0 up
  ip address add 192.168.123.1/24 dev quantum-br0
...
@test "Create tap devices" {
  for i in 1 2 3; do
    ip link del quantum-srv${i} 2>/dev/null || true
    ip tuntap add dev quantum-srv${i} mode tap
    ip link set quantum-srv${i} up
    ip link set quantum-srv${i} master quantum-br0
  done
}
...
@test "Boot QEMU VMs" {
  for i in 1 2 3; do
    qemu-system-x86_64 -machine type=pc,accel=kvm -cpu host -smp 8 -m 24576 \
      ...
      -netdev tap,id=net0,ifname=quantum-srv${i},script=no,downscript=no \
...
cluster:
  apiServer:
    extraArgs:
      oidc-issuer-url: "https://keycloak.example.org/realms/quantum"
...
EOF
  ...
  talosctl gen config --with-secrets secrets.yaml quantumreasoning https://192.168.123.10:6443 \
