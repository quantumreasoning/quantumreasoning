...
@test "Create a VM Disk" {
  name='test'
  kubectl -n tenant-test get vmdisks.apps.quantumreasoning.io $name || 
...
@test "Create a VM Instance" {
  diskName='test'
  name='test'
  kubectl -n tenant-test get vminstances.apps.quantumreasoning.io $name || 
  kubectl create -f - <<EOF
apiVersion: apps.quantumreasoning.io/v1alpha1
kind: VMInstance
metadata:
...
EOF
  ...
  kubectl -n tenant-test delete vminstances.apps.quantumreasoning.io $name 
  kubectl -n tenant-test delete vmdisks.apps.quantumreasoning.io $diskName 
}
