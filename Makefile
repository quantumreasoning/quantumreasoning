...
build: build-deps
	...
	make -C packages/system/quantumreasoning-api image
	make -C packages/system/quantumreasoning-controller image
...
manifests:
	mkdir -p _out/assets
	(cd packages/core/installer/; helm template -n quantum-installer installer .) > _out/assets/quantumreasoning-installer.yaml
