...
func (r *TenantHelmReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	for _, item := range hrList.Items {
		...
		oldDigest := item.GetAnnotations()["quantumreasoning.io/tenant-config-digest"]
		...
		patchTarget.Annotations["quantumreasoning.io/tenant-config-digest"] = newDigest
...
func annotateTenantRootNs(values apiextensionsv1.JSON, c client.Client) error {
	...
	ns.Annotations["namespace.quantumreasoning.io/host"] = host
