...
import (
	...
	quantumv1alpha1 "github.com/quantumreasoning/quantumreasoning/api/v1alpha1"
)
...
func (c *Collector) collect(ctx context.Context) {
	...
	var quantumreasoningCM corev1.ConfigMap
	if err := c.client.Get(ctx, types.NamespacedName{Namespace: "quantum-system", Name: "quantumreasoning"}, &quantumreasoningCM); err != nil {
		logger.Info(fmt.Sprintf("Failed to get quantumreasoning configmap in quantum-system namespace: %v", err))
		return
	}

	oidcEnabled := quantumreasoningCM.Data["oidc-enabled"]
	bundle := quantumreasoningCM.Data["bundle-name"]
	bundleEnable := quantumreasoningCM.Data["bundle-enable"]
	bundleDisable := quantumreasoningCM.Data["bundle-disable"]
	...
	// Add quantumreasoning info metric
	if len(nodeList.Items) > 0 {
		k8sVersion, _ := c.discoveryClient.ServerVersion()
		metrics.WriteString(fmt.Sprintf(
			"quantum_cluster_info{quantumreasoning_version=\"%s\",kubernetes_version=\"%s\",oidc_enabled=\"%s\",bundle_name=\"%s\",bunde_enable=\"%s\",bunde_disable=\"%s\"} 1\n",
			c.config.quantumreasoningVersion,
			k8sVersion,
			oidcEnabled,
			bundle,
			bundleEnable,
	...
	for osKey, count := range nodeOSCount {
		metrics.WriteString(fmt.Sprintf(
			"quantum_nodes_count{os=\"%s\",kernel=\"%s\"} %d\n",
			osKey,
	...
	if err := c.client.List(ctx, &serviceList); err != nil {
		...
		metrics.WriteString(fmt.Sprintf("quantum_loadbalancers_count %d\n", lbCount))
	}
	...
	if err := c.client.List(ctx, &nsList); err != nil {
		...
		metrics.WriteString(fmt.Sprintf("quantum_tenants_count %d\n", tenantCount))
	}
	...
	if err := c.client.List(ctx, &pvList); err != nil {
		...
		for size, drivers := range pvMetrics {
			for driver, count := range drivers {
				metrics.WriteString(fmt.Sprintf(
					"quantum_pvs_count{driver=\"%s\",size=\"%s\"} %d\n",
					driver,
					size,
					count,
				))
	...
	var monitorList quantumv1alpha1.WorkloadMonitorList
	...
	for _, monitor := range monitorList.Items {
		metrics.WriteString(fmt.Sprintf(
			"quantum_workloads_count{uid=\"%s\",kind=\"%s\",type=\"%s\",version=\"%s\"} %d\n",
			monitor.UID,
			monitor.Spec.Kind,
			monitor.Spec.Type,
